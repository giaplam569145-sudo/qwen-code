...
const GOOGLE_DIR = '.qwen';
const GOOGLE_CREDENTIAL_FILENAME = '.google_oauth_creds.json';
...
async function initOauthClient(
  ...
  if (await loadCachedCredentials(client)) {
    console.log('Loaded cached Google credentials.');
    return client;
  }
...
function getGoogleCachedCredentialPath(): string {
  return path.join(os.homedir(), GOOGLE_DIR, GOOGLE_CREDENTIAL_FILENAME);
}

async function loadCachedCredentials(client: OAuth2Client): Promise<boolean> {
  const keyFile = getGoogleCachedCredentialPath();
  try {
...
async function cacheCredentials(credentials: Credentials) {
  const filePath = getGoogleCachedCredentialPath();
...
export async function clearGoogleCachedCredentialFile() {
  try {
    await fs.rm(getGoogleCachedCredentialPath(), { force: true });
